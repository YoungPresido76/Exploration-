...
const server=createServer((_req:IncomingMessage,res:ServerResponse)=>{res.writeHead(200,{'content-type':'application/json','access-control-allow-origin':'*','cache-control':'no-store'});res.end(JSON.stringify({service:'pixel-arena-authoritative-server',status:'ready',tickRate:30,rooms:[...rooms.keys()]}))});
...
if(process.env.NODE_ENV!=='test'){const port=Number(process.env.PORT||8787);server.listen({port,host:'0.0.0.0'},()=>console.log(`Pixel Arena authoritative server listening on ${port}`));server.on('error',(error)=>{console.error('Pixel Arena server error',error);process.exitCode=1})}
